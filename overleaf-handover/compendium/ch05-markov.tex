...
Suppose $a_1$ is drawn with $\Var(a_1)=\sigma^2$ and ask when that variance is preserved along
the chain. Taking variances in \eqref{eq:ar1} and using independence of $\varepsilon_i$ from
$a_{i-1}$,
\[
...
\label{rem:cov-stationary}
The computation above establishes exactly one thing: with $\ivar=1-\corr^2$ and
$\Var(a_1)=1$, \emph{second} moments propagate unchanged, so $\Var(a_i)=1$ for every $i$ and
$\Cov(a_i,a_j)=\corr^{|i-j|}$, whatever the shape of $\innov$. The chain is
\textbf{covariance-stationary} (equivalently second-order stationary).

It is \textbf{not} strictly stationary in distribution unless $\innov$ is Gaussian. The
invariant law of $a_i = \corr a_{i-1} + \varepsilon_i$ is the law of
$\sum_{k\ge0}\corr^k\varepsilon_{-k}$, an infinite convolution which for non-Gaussian
$\varepsilon$ is not Gaussian; drawing $a_1\sim\normal{0}{1}$ therefore starts the chain off
its invariant law, and the marginal of $a_i$ drifts towards that law over a burn-in of order
$1/\log(1/\corr)\approx6$ sites at $\corr=0.85$. Choosing $\ivar=1-\corr^2$ alone does not
buy strict stationarity, and this document does not claim it does.

Nothing in the comparisons depends on the difference. Every family shares the same $a_1$ law
and the same covariance, so they still differ only beyond second moments, which is the property
the controlled probe needs. But the alternative --- sampling $a_1$ from each family's invariant
law, which needs a fixed-point computation per family --- has not been run, so the claim that it
would change nothing is a prediction and not a measurement. It is on the list in
\S\ref{sec:pending}.
\end{remark}
