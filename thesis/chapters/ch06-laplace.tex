...
\includegraphics[width=0.8\linewidth]{figures/fig06_K1_density.pdf}
...
The score--posterior identity \eqref{eq:tweedie} holds for \emph{any}
prior: $S(x, t) = (\mu\, \E[a|x] - x)/\Deltat$. For a Gaussian prior the
...
(Figure~\ref{fig:l-score-limits}): for every fixed $x \neq 0$,
\begin{equation}
...
\end{equation}
that is: at small $t$ the score converges pointwise to the
\emph{sign-shaped} score of the Laplace prior itself, with all the
...
the posterior-expectation route against central finite differences of
$\log p_t^{(1)}$ computed by quadrature: maximum disagreement
$1.19 \times 10^{-7}$ over a grid of $(b, t, x)$, at the truncation floor
of the finite difference; the Gaussian-attractor limit at $t = 6$ holds to
$7.4 \times 10^{-6}$.

\begin{figure}[t]\centering
\includegraphics[width=0.8\linewidth]{figures/fig07_K1_score.pdf}
...
\begin{figure}[t]\centering
\includegraphics[width=\linewidth]{figures/fig08_K1_score_limits.pdf}
...
the \emph{same} tree-shaped factor graph as the Gaussian model (a fact
made precise in Chapter~\ref{ch:bp}): belief propagation remains exact in
...
dynamics; the $K = 1$ analysis of $p_t^{(1)}$ (screened Poisson equation
...
quadrature audits quoted above (posterior-expectation route vs finite
differences; characteristic function vs direct integration). \emph{Open}: a
...
Laplace chain is structurally identical to the Gaussian one --- the same
tree, the same two-sweep message schedule, the same score--posterior
identity --- but its messages are genuinely infinite-dimensional functions,
so the number of message \emph{updates} stays linear in $K$ while the cost
of each update is no longer bounded. The question ``what does it cost to
force the messages Gaussian?'' is now sharply posed; Chapter~\ref{ch:bp}
answers it.
