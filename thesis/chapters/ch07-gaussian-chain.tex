...
frame equal to $1$ (Toolbox~3.1). One should think of $k$ as an
...
This was derived in Toolbox~3.1 (unrolling the recursion); stationarity is
...
variables (the Hammersley--Clifford fingerprint of
...
Tweedie's identity \eqref{eq:tweedie-joint} expresses the same score through
the posterior mean of the clean chain:
\begin{equation}
  \score_k(x, t)
...
\section{The lifecycle of the precision matrix}\label{sec:g-lifecycle}

How does $\Qt$ interpolate between its two known endpoints --- the
tridiagonal $\Qz$ at $t = 0$ and the identity at $t = \infty$? Three
equivalent representations organise the answer.

\begin{theorem}[Three forms of $\Qt$]\label{thm:g-three-forms}
For all $t \geq 0$, with $\gamma_t := e^{-2t}/\Deltat$ (the channel SNR) and
$c_t := e^{2t} - 1$:
\begin{align}
  \Qt &= \tfrac{1}{\Deltat} \big( I + \gamma_t\, \Sigz \big)^{-1}
  && \text{(SNR form)}, \label{eq:g-Qt-snr}\\[2pt]
  \Qt &= U \operatorname{diag}\!\Big(
        \frac{1}{e^{-2t}\omega_i + \Deltat} \Big) U^\T,
  \quad \Sigz = U \operatorname{diag}(\omega_i)\, U^\T
  && \text{(spectral form)}, \label{eq:g-Qt-spec}\\[2pt]
  \Qt &= e^{2t}\, \Qz \big( I + c_t \Qz \big)^{-1}
      = e^{2t} \sum_{m \geq 0} (-c_t)^m\, \Qz^{\,m+1}
  && \text{(resolvent form)}. \label{eq:g-Qt-res}
\end{align}
\end{theorem}

\begin{toolbox}{Deriving the three forms}
\emph{SNR form:} factor $\Deltat$ out of \eqref{eq:g-Sigt},
$\Sigt = \Deltat(I + \gamma_t \Sigz)$, and invert.
\emph{Spectral form:} $\Sigz$ is symmetric, so the spectral theorem gives an
orthonormal eigenbasis $\Sigz = U \operatorname{diag}(\omega_i) U^\T$; since
$I$ commutes with everything,
$\Sigt = U \operatorname{diag}(e^{-2t}\omega_i + \Deltat) U^\T$, and
inverting a diagonal matrix is entrywise.
\emph{Resolvent form:} multiply \eqref{eq:g-Sigt} on the left by $\Qz$ and
use $\Deltat e^{2t} = c_t$:
$\Qz \Sigt = e^{-2t}(I + c_t \Qz)$; invert both sides. The Neumann series
converges when $\|c_t \Qz\| < 1$, i.e.\ in the small-$t$/high-SNR regime,
which is exactly where we deploy it below. \hfill$\square$
\end{toolbox}

The spectral form contains a structural fact of independent interest:
\emph{the eigenvectors of $\Sigt$ and $\Qt$ are those of $\Sigz$, for every
$t$} --- only the eigenvalues flow, $\omega_i \mapsto e^{-2t}\omega_i +
\Deltat \to 1$ (Figure~\ref{fig:g-spectral}). Because $\Sigz$ is symmetric
Toeplitz, its eigenvectors are near-Fourier modes: in frequency space the
denoising problem decouples into $K$ scalar problems at all times, while in
the \emph{frame} basis --- the operationally meaningful one for sequences ---
$\Qt$ is sparse only at $t = 0$. Locality is a property of a matrix
\emph{in a basis}, and the two statements are complementary, not in tension.
...
as $e^{-2t}\omega_i + \Deltat$, all monotonically towards $1$. Right: the
eigenvectors of $\Sigz$ --- shared by $\Sigt$ and $\Qt$ at every $t$ --- are
near-Fourier modes of the chain. Figure generated by the companion code
...
$\alpha = 0.8$): exactly tridiagonal at $t = 0$ (Markov structure), the band
fills at intermediate $t$ (every frame couples to every other), and the
matrix collapses to the identity as $t \to \infty$ (isotropic noise wins).}
\label{fig:g-lifecycle}
\end{figure}

\subsection{The band-fill law at small $t$, corrected}\label{sec:g-bandfill}

At $t = 0$ all entries of $\Qt$ at distance $d \geq 2$ vanish. How fast do
they switch on?
...
\begin{figure}[t]\centering
\includegraphics[width=0.78\linewidth]{figures/fig_band_fill.png}
...
\subsection{Return to isotropy at large $t$}

Writing $\Sigt = I + e^{-2t}(\Sigz - I)$ and expanding the inverse,
\begin{equation}
  \Qt = I - e^{-2t}(\Sigz - I) + e^{-4t}(\Sigz - I)^2 + O(e^{-6t}),
  \label{eq:g-larget}
\end{equation}
so $\| \Qt - I \|_F \simeq e^{-2t} \|\Sigz - I\|_F$: all structure dies at
the universal rate $e^{-2t}$, and the score collapses onto the isotropic
$-x$. The full lifecycle is thus: \emph{sparse} (tridiagonal, Markov) at
$t = 0$; \emph{dense} at intermediate $t$, with off-diagonals switching on
at rate $t^{d-1}$ per diagonal $d$; \emph{isotropic} at $t \to \infty$, with
all structure decaying as $e^{-2t}$ (Figure~\ref{fig:g-tridiagloss}).

\begin{figure}[t]\centering
\includegraphics[width=0.78\linewidth]{figures/fig_tridiag_loss.png}
...
are exactly right:
\begin{equation}
  t \to 0:\;\; q \approx \frac{|\beta|}{J_d} \to 0
  \quad (\text{each frame pinned by its own data});
  \qquad
  t \to \infty:\;\; q \to |\alpha|
  \quad (\text{posterior} \to \text{prior}),
  \label{eq:g-q-limits}
\end{equation}
...
$-\Qt x$ with $\Qt = (e^{-2t}\Sigz + \Deltat I)^{-1}$, equal by Tweedie to
the posterior-mean form \eqref{eq:g-tweedie} (both audited to machine
precision); the clean precision is tridiagonal (Markov), the posterior
...
Chapter~\ref{ch:bp} rebuilds the same score from local message passing at
$O(K)$ --- after Chapter~\ref{ch:laplace} shows what changes when
Gaussianity, and with it linearity, is abandoned.
