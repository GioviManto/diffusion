...
matching on identical data, across the \nsizesused{} training-set sizes tested.
The ratio is \emph{not} monotone in the training-set size, and an earlier draft
of this chapter said it rose with it; the reported summary is also the largest
of several defensible aggregations of the same cells, which
Appendix~\ref{app:aggregation} sets out. At the larger sizes each side
...
assumption and not as its size. How much survives against a baseline that can
actually represent chain inference, rather than one restricted to a fixed
window, is measured in Chapter~\ref{ch:em-results} and is materially smaller;
that measurement is the one to quote, and its own limits are stated where it
appears.
...
fits. The capacity statement additionally overreached in a second way, which
is worth separating because it would have survived a converged rerun: failing
to resolve a difference between $C=8$ and $C=16$ at six seeds is not evidence
that none exists, and nothing here predeclared how small a difference would
count as none. Both results are recorded in
Appendix~\ref{app:withdrawn} as methodological negative results. They are reported because a budget-limited comparison answers ``how do
