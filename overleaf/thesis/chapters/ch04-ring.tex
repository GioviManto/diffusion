...
This is where the work started, and it is placed first because what it settles
is the reason for everything that follows.
...
scalar chain precisely because rotation is the simplest dynamics whose
signature is not visible in a single frame.

That last point turns out to be a theorem. The rotation rate carries
\emph{zero} Fisher information in every one-frame marginal, and positive
information in the trajectory. A model that sees frames one at a time cannot
estimate it at all, however many frames it sees and however long it is trained;
a model that sees the trajectory can. Chapter~\ref{ch:model} gives the
corresponding statement for the stationary Gaussian chain, where the marginals
are $\alpha$-free outright (Proposition~\ref{prop:dev-marginal}), but the
argument there does not extend beyond Gaussian innovations. Here the conclusion
is exact, and it holds because of a symmetry rather than a calculation that
happens to work out.

This is the argument for studying the \emph{joint} score, and it is what sends
the rest of the thesis to a model in which the joint score can be computed in
full. The ring itself is only partly tractable: a Cartesian surrogate is exactly
solvable, and the model as drawn is solvable numerically to a measured accuracy.
Chapters~\ref{ch:gaussian} onwards trade this model's realism for a scalar
chain in which every quantity has a closed form.
...
Give the initial phase the uniform law on the circle. Then every single-frame
marginal is independent of the rotation, at every noise level. This holds for
the polar model
\eqref{eq:ring-radial-sde}--\eqref{eq:ring-embedding} in its parameter
$\omega$, and for the surrogate \eqref{eq:ring-sur-dynamics} in its parameter
$\psi$. Consequently a model built from per-frame marginals carries exactly
zero Fisher information about the rotation.
\end{theorem}

\begin{proof}
\emph{The polar model.} The radial and angular diffusions
\eqref{eq:ring-radial-sde}--\eqref{eq:ring-angular-sde} are independent, and
integrating the second gives
$\Theta_u = \Theta_0 + \omega u + \sqrt{2D_\theta}\, B^{(\theta)}_u$. The initial
phase is uniform on the circle and independent of everything added to it, and
convolving the uniform law on a compact group with any independent law returns
the uniform law; so $\Theta_u$ is uniform for every $u$, whatever $\omega$ is.
Since $R_u$ carries no $\omega$ and is independent of $\Theta_u$, the law of
$a_u = R_u(\cos\Theta_u, \sin\Theta_u)$ does not depend on $\omega$. The
confining drift is irrelevant to the argument precisely because it acts on the
radius alone.

\emph{The surrogate.} Here the dynamics are linear and the same conclusion
follows by unrolling rather than by symmetry of the phase. Unrolling the
recursion,
...
The theorem furnishes an unusually clean experiment, because this model carries
both a parameter the marginals can see, the confining potential
$V(r) = (r-r_*)^2/2\lambda$, and one they provably cannot. Running both through
the same estimator isolates the effect of the structure and not of the method.
The marginal-visible potential is the control that makes the contrast readable;
no measurement of it is reported here. What follows is the two targets the
marginals cannot see: a single rotation $\psi$, and a law over rotations
$p(\psi)$.
...
confident, wrong answer and not an error. It is recorded here because the
...
\section{Derivations}\label{app:ring}\label{sec:ring-derivations}

The four calculations the chapter relies on, in the order it uses them.
...
\subsection{Rotations preserve the standard isotropic Gaussian}
...
\subsection{The gauge reduction}
...
\subsection{The exact surrogate score}
...
\begin{enumerate}
  ...
  theorem and not on a matched pair of measurements. Nothing else in the
