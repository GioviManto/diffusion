...
the grid resolution $M$. The gap between the two rows matters, because a figure
...
finite spacing $h=2A/(M-1)$ --- the grid holds $M$ points including both
...
diagnostic: at the working configuration the worst column with
$|a| \le A/2$ --- far enough inside the domain that its transition density has
no tail outside the grid, so the residual measures quadrature alone --- departs
from unit mass by $\gdinterior$, while the worst column anywhere, at the domain
edge where the density is truncated, departs by $\gdedge$.
...
kernel, but its normaliser would depend on $\theta$, most strongly near the
...
\includegraphics[width=0.72\linewidth]{fig_grid_domain.pdf}
\caption[Grid and domain control, separated]{Grid and domain control on a Laplace chain ($\alpha=0.85$, $n=32$
sites), four panels. (a) Relative score error against a high-resolution
reference as grid resolution $M$ grows at fixed half-width $A=8$; (b) the same
as $A$ grows at fixed $M=401$ --- both saturate at the floating-point floor
...
\begin{figure}[t]\centering
\includegraphics[width=0.9\linewidth]{fig_em_diagnostics.pdf}
...
Table~\ref{tab:em-convergence} fits two arms of the same chains under identical
initialisation and budget, one seeing clean data and the other the same chains
once through the OU channel:

\begin{center}
\begin{minipage}{\linewidth}\centering
\captionof{table}[What the channel costs in convergence rate]{What the observation channel costs in convergence rate, not in attainable accuracy. Two arms of the same chains, identical initialisation and budget, one seeing clean transition pairs and the other the same chains once through the OU channel (generating values: $\alpha = 0.85$, innovation kurtosis $3.0$). Both arms approach the same fitted summaries within this experiment's resolution; what the channel changes is the rate. A finite mixture does not contain the generating law exactly, so what is approached is the best fit within the family rather than the law itself.}
...
channel. The channel compounds this. A dedicated sweep measured the cost
directly, recording for each fit the first iteration after which a coordinate
stays within a fixed relative tolerance of its final value. At
$\nseq = \convsize$, $C = \convcomps$ and the \convdesign{} noise design, over
$\convseeds$ seeds run to $\convcap$ iterations, the correlation coefficient
settles at a median of $\convrhomed$ iterations and the innovation shape at
...
is the one reported, because the two coordinates are measured on the same fit.
Two things about what is being measured. ``Settled'' means the first update
after which the coordinate stays within a fixed relative tolerance of its value
at the end of the run --- $10^{-3}$ for $\alpha$, $2\times10^{-2}$ for the
excess kurtosis --- so it is a statement about the stability of a trace, not
about distance to an optimiser. And the run has to be long enough for that to
mean anything: the sweep was run at two lengths, and these medians come from the
$\convcap$-update shard, the only one in which no configuration settles near its
own cap. Against those numbers the fixed budget of $\convbudget$ iterations that
several experiments in Chapter~\ref{ch:em-results} used as a default, before
this measurement existed, is short of the shape coordinate's median by about a
factor of six, and leaves $\convunsettled$ of the $\convunsettledof$
configurations in the sweep --- both shards, since censoring can only shorten a
settling time and so cannot inflate this count --- unsettled in it.
...
fixed budget of 200 iterations, there is no channel penalty on converged
shape recovery: the paired clean $-$ noised difference is
$-0.052\pm0.217$, consistent with zero. A single non-converged draw would
