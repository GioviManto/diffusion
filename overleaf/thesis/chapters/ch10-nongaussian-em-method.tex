...
\captionof{table}[What the channel costs in convergence rate]{What the observation channel costs in convergence rate, not in attainable accuracy. Two arms of the same chains, identical initialisation and budget, one seeing clean transition pairs and the other the same chains once through the OU channel (truth: $\alpha = 0.85$, innovation kurtosis $3.0$). Both arms reach the truth; only the rate differs.}
...
$\convshapemed$, with a tail to $\convshapemax$ in the slowest seed --- a paired
median factor of $\convratio$ between the two coordinates. Measured against
that, the fixed budget of $\convbudget$ iterations that several of the
experiments in Chapter~\ref{ch:em-results} used as a default, before this
measurement existed, is short of the shape coordinate's median by about a factor
of six, and leaves $\convunsettled$ of the sweep's $\convunsettledof$
configurations unsettled in it.
