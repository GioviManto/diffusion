...
diagnostic: at the working configuration the worst interior column departs from
unit mass by $9.6\times10^{-4}$, and the worst column anywhere, at the domain
edge where the density is truncated, by $1.9\times10^{-2}$.
...
truncation error for every run, and the gap is large enough to matter: over 256
independent chains per configuration the single-chain value sits near the
\emph{median}, while the maximum observed is up to $152\times$ higher (range
$2.9\times$ to $8369\times$ across configurations). This is why the figures
above are reported as a maximum and a $90$th percentile over chains rather than
as one number. The diagnostic is an empirical estimate of typical truncation
