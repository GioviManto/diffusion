...
The thread is one idea, restated four times. A physical trajectory is a
stationary point of the action (\S\ref{sec:action}). An equilibrium
distribution is a stationary point of the free energy
(Appendix~\ref{app:statmech}). A diffusion model is trained by making a
variational bound stationary (Appendix~\ref{app:diffusion-deriv}). And the
transition kernel of Chapter~\ref{ch:em-method} is estimated by making a
likelihood stationary. The objects change; the shape of the argument does not,
and that continuity is why this thesis is written in the physics tradition
rather than only borrowing its vocabulary.
