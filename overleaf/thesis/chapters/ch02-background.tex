...
\compendium{}, which is unbounded in length; the body's job is the thread
connecting them.
...
point of the free energy; a diffusion model is
...
starting there rather than with Hamilton's equations. The same statement ---
\emph{a physical trajectory is a stationary point of a functional} --- returns
three more times in this thesis: as the free-energy principle of
thermodynamics, as the variational bound that trains a diffusion model
(\S\ref{sec:diffusion-idea}), and as the maximum-likelihood problem solved by
expectation--maximisation in Chapter~\ref{ch:em-method}. The objects change; the shape of the argument
does not.

Let a system be described by generalised coordinates $q(t)$, let the
\emph{Lagrangian} $L(q,\dot q,t)$ be the difference between kinetic and
potential energy, and define the \emph{action} of a path joining fixed
endpoints,
\begin{equation}
  S[q] \;=\; \int_{t_1}^{t_2} L\big(q(t), \dot q(t), t\big)\,\dd t .
  \label{eq:action}
\end{equation}
The action assigns one number to a whole path, and Hamilton's principle states
that the path actually taken is a \emph{stationary point} of it: $\delta S = 0$
for every smooth variation vanishing at the endpoints. Carrying out that
variation gives the Euler--Lagrange equations, which for
$L = \sum_i \tfrac12 m_i\dot q_i^2 - U(q)$ return Newton's second law. What
generalises is not the equations but the move: dynamics derived from a scalar
functional made stationary.

\begin{remark}[Stationary, not minimal]
It is usually called the principle of \emph{least} action, and that is slightly
wrong. The physical path is stationary, not necessarily minimal --- it can be a
saddle. The same caution applies everywhere the pattern recurs:
expectation--maximisation converges to a stationary point of the likelihood, not
to a global maximum (Chapter~\ref{ch:em-method}), and reading ``the algorithm
converged'' as ``the algorithm found the best answer'' is a recurring source of
false confidence.
\end{remark}
...
\citet{hopfield1982neural} wired $N$ neurons to $P$ patterns by the Hebb rule
\citep{hebb1949organization}, giving \eqref{eq:ising} with
$J_{ij} = N^{-1}\sum_\mu \xi_i^\mu \xi_j^\mu$, and aligned each neuron with its
local field. Two lines of algebra show that dynamics descends $H$ at every flip,
so every initial condition falls into an attractor and retrieval \emph{is} that
fall. How many memories fit is a phase-transition question: for random patterns
with $P = \alpha N$, retrieval survives only below $\alpha_c \approx 0.138$
\citep{amit1985storing}, beyond which they are crushed into a glassy phase.
Memory is a phase of matter here in the technical sense --- entered and left
through sharp transitions. The 2024 Nobel Prize in Physics went to Hopfield and
Hinton \citep{nobel2024physics}, recognising the lineage as physics rather than
as physics-inspired.
...
maximum-entropy principle turned into an algorithm,
...
only by sampling. Restricting the couplings to a
...
One caveat travels with all of this and is used later. A phase transition is a
statement about the thermodynamic limit, so in the finite models of
...
(Table~\ref{tab:statmech-dictionary}):
\begin{center}
\begin{minipage}{\linewidth}\centering
\captionof{table}[Statistical mechanics and generative modelling, term by term]{The dictionary this chapter builds. Each row is an identification used somewhere in the thesis, not an analogy: the left column names the statistical-mechanical object and the right the generative-modelling one it is the same object as.}
\label{tab:statmech-dictionary}
\begin{tabular}{ll}
\toprule
...
\noindent The running example of this thesis is the scalar Gaussian
...
\begin{equation}
  ...
  \label{eq:ar1}
\end{equation}
whose transition kernel is $M(a' \mid a) = \Nd(a'; \alpha a, \ivar)$. Taking
...
Chapter~\ref{ch:gaussian} onwards is about \eqref{eq:ar1}, its non-Gaussian
variants, or what an estimator can recover of it through noise.
...
invoked above, with the physical dynamics
...
Two structural remarks, both used repeatedly later. First, the OU kernel is
...
\end{equation}
up to time $T$, producing marginals $p_t$ whose evolution is governed by the
...
\end{equation}
The Ornstein--Uhlenbeck channel \eqref{eq:ou-sde} is the case
$f = -x$, $g = \sqrt{2}$. Can the
...
an energy-based model: no energy is ever normalised, no equilibrium
...
perturbations converting a structured distribution into a simple one --- an
annealing schedule. The objective they derive is then a variational bound of
...
diffusion models with the tools of \S\S\ref{sec:action}--\ref{sec:statmech-learning}, in the
...
sequence of symmetry breakings in the student's singular modes. An RBM
learning a Hopfield teacher with two correlated patterns then undergoes not one
transition but a \emph{cascade}: each singular direction condenses at its own
critical time, in order of the strength of the data covariance along it, and
each condensation is a second-order transition of Curie--Weiss type. The mapping onto Curie--Weiss, the two-rate argument that
produces the ordering, and the confirmation on MNIST, CelebA and human-genome
data are in \compendium{}.
