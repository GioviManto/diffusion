...
A random vector $a \in \R^L$ is Gaussian with mean $m$ and (positive
definite) covariance $\Sigma$, written $a \sim \Nd(m, \Sigma)$, if its
density is
\begin{equation}
  \Nd(a;\, m, \Sigma)
  \;=\; \frac{1}{(2\pi)^{L/2} \sqrt{\det \Sigma}}
...
\label{thm:spectral}
Every symmetric $A \in \R^{L \times L}$ has an orthonormal basis of
eigenvectors: $A = U \operatorname{diag}(\omega_1, \dots, \omega_L)
...
Gaussian problems decouple, in the large-$L$ limit, into independent
