...
objective (Chapter~\ref{ch:diffusion-long}), and sampling by simulating a
...
diffusion model (Chapter~\ref{ch:diffusion-long}), and as the maximum-likelihood
problem solved by expectation--maximisation in
Chapter~\ref{ch:estimation}. The objects change; the shape of the argument
does not.
...
not to a global maximum (Chapter~\ref{ch:estimation}), and it is a recurring
...
recursion that Chapter~\ref{ch:graphical} presents as belief propagation.
...
Chapter~\ref{ch:graphical} develops and that the research chapters deploy on
...
dynamics. Chapter~\ref{ch:diffusion-long} follows that branch from
...
\section{The correspondence, term by term}\label{sec:statmech-dictionary}

The table below collects the correspondences this chapter builds
(Table~\ref{tab:statmech-dictionary}). They are not all of the same kind, and
the difference matters. Some rows are exact identifications under a stated
convention: a Boltzmann distribution at $\beta = 1$ with $H = -\log p$ is the
model distribution, and the partition function is its normalising constant.
Others are correspondences that hold up to qualifications made where they are
used: the free energy equals the negative ELBO only up to constants and a
choice of variational family; Langevin dynamics and reverse diffusion coincide
only for the specific drift of Anderson's theorem, not in general; and
``phase transition'' names a family of distinct phenomena --- feature
condensation, speciation, memorisation --- which share a mathematical signature
but not a mechanism. The table is a reading aid for moving between two
literatures, not a list of theorems.

\begin{center}
\begin{minipage}{\linewidth}\centering
\captionof{table}[Statistical mechanics and generative modelling, term by term]{Correspondences between the statistical-mechanical and generative-modelling vocabularies. The rows are of mixed strength: some are exact identifications under the conventions fixed above, others hold up to constants or only in the special cases noted in the text.}
