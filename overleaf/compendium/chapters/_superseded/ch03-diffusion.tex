...
The Ornstein--Uhlenbeck (OU) process satisfies all three. In the variance-preserving
convention used throughout \citep{song2021scorebased}, it is the stochastic differential equation
\begin{equation}
\label{eq:ou}
...
\coderef{src/noising.py}{alpha\_delta} for the coefficient pair.
\end{codebox}
...
\begin{pitfall}
Integration cannot be carried to $t=0$, but not for the reason usually given. The score
\eqref{eq:score-final} carries a factor $1/\Dt$, and it is tempting to conclude that it
diverges. It generally does not: the numerator $x - e^{-t}\pmean{a}$ vanishes at the same
order, and for a $\Pz$ with a continuously differentiable density
$\score(x,t)\to\nabla\log\Pz(x)$, which is finite. What actually forces $\tmin>0$ is a
combination of four effects, separated in \S\ref{sec:resolution}: a genuine singularity for
compactly supported innovations only; cancellation in a difference of nearly equal
quantities divided by $\Dt$; a likelihood factor of width $\sqrt{\Dt}$ that the grid stops
resolving; and a reverse drift whose Jacobian scales like $1/\Dt$, which stiffens the
integrator. Whatever the reason, the consequence is the same: what the sampler produces is a
sample of $P_{\tmin}$, \emph{not} of $\Pz$, and comparing it against clean data measures the
stopping point rather than the score. That mistake was made twice in this project before the
forward-noised reference of \S\ref{sec:tmin} fixed it.
\end{pitfall}

\section*{Sources}
\addcontentsline{toc}{section}{Sources}
The variance-preserving forward process and the SDE formulation of diffusion generative models
are from \citet{song2021scorebased}; the discrete-time antecedents are \citet{sohl2015deep} and
\citet{ho2020denoising}. The time-reversal theorem that supplies the reverse drift is
\citet{anderson1982reverse}, with the general treatment in \citet{haussmann1986}. The Fokker--Planck
machinery underlying both is standard \citep{risken1989}. Choices of $\tmin$, $\tmax$ and the
time discretisation are discussed empirically by \citet{karras2022elucidating}.
