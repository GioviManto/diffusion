...
The \emph{shape-based} capacity effect is a convergence artefact & withdrawn &
  ...
  or generated shape \\
Held-out evidence saturates near eight components & conditional &
  \emph{survives} \S\ref{sec:corr-capacity}: it is a separate paired design
  (six seeds, $\nseq=128$) scored on held-out evidence, which settles early in
  the optimisation, not on fitted shape. Conditional on that metric --- it is
  not a claim about the converged \emph{shape} \\
...
radius $2$. Confirmed at the headline protocol on the full sixteen frozen
seeds, all-site scored: EM--BP is more accurate than the window head in every
one of $192$ scored (seed, size, region) cells, by a factor rising from
$2.34\times$ at $\nseq=32$ to $6.14\times$ at $\nseq=2048$ --- monotone
\emph{increasing} with $\nseq$, unlike the unstructured comparison, which is
consistent with the estimator's structure paying off more precisely as it has
more data to use it on while the window's receptive field stays fixed. This
...
\item \textbf{Capacity equivalence at a sufficient budget}
      (\codefile{experiments/exp\_32\_capacity\_equivalence.py}). The rerun that would settle
      the withdrawn capacity attribution below, now with a predeclared equivalence region and
      ...
      $10^{-9}$ tolerance is reachable. Jobs \texttt{641515}--\texttt{641520}; lanes~a and~c are
      complete ($70/70$, $40/40$), lane~b reached $39/50$ before two consecutive six-hour shards
      both ran out of wall time and a third resume shard (job \texttt{643778}) is in flight ---
      \texttt{part\_sweep}'s per-cell flush and its own on-disk resume check make a third shard
      safe to run against the same output directory rather than overwriting the first thirty-nine
      cells.
