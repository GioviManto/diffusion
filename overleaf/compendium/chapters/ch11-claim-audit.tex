...
\item \textbf{The structure-aware baseline}
      (\codefile{experiments/exp\_31\_structured\_baseline.py}). The measurement an outside
      review identified as the outstanding one: what survives of the headline ratio against a
      baseline that can represent chain inference, rather than the fixed window that closes most
      of the gap. Runs the headline protocol throughout --- all-site and predeclared-interior
      scoring alongside the centre-site diagnostic, both arms checkpointed and selected on
      validation --- and adds dilated-stack and bidirectional message-passing arms, which a window
      head cannot match however much data it sees. Deployed through
      \codefile{hpc/deploy\_clean.sh}, so its provenance is verifiable by construction, unlike
      the \texttt{exp\_12\_scaled} comparison it replaces. Three seed-range lanes, jobs
      \texttt{641509}--\texttt{641514}. Until it lands, no quantitative structure-aware claim
      appears in any document; the note and the thesis carry the direction and its order only.
...
\item \textbf{Capacity equivalence at a sufficient budget}
      ...
      $10^{-9}$ tolerance is reachable. Jobs \texttt{641515}--\texttt{641520}; lane~a is complete
      at $70/70$ cells, lanes~b and~c still running.
