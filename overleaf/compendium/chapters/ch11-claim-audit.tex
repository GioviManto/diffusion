...
\paragraph{Result, at eight seeds (\texttt{629962}, 432 cells).} The joint arm recovers the
well width better than the marginal arm --- median relative error $0.284$ against $0.380$ ---
and its accuracy improves monotonically with the number of trajectories:
...
\midrule
median $|\hat\lambda-\lambda|/\lambda$ & $0.726$ & $0.367$ & $0.260$ & $0.175$ & $0.161$ \\
\bottomrule
\end{tabular}
\end{center}
...
both converged. With the per-sequence tolerance and a raised cap the rates are $43.1\%$ and
$21.8\%$, and the comparison is between estimators rather than between budgets.

\begin{pitfall}
Two caveats travel with the headline. The joint arm's advantage is confined to
$t \lesssim 0.5$: at $t \geq 0.98$ the marginal arm is \emph{better}, and quoting a
schedule-wide margin would hide that. And the median errors are large in absolute terms ---
$0.16$ at the largest sample size is not a recovered parameter, it is a trend. What this rung
supports is the ordering and the scaling, not a precision claim.
\end{pitfall}
