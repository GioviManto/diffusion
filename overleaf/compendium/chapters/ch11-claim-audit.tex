...
{\small
\begin{longtable}{@{}p{0.30\textwidth}p{0.13\textwidth}p{0.47\textwidth}@{}}
\toprule
...
Structure is worth an order of magnitude against networks & conditional &
  \S\ref{sec:rec-efficiency}; conditional on the arms being asymmetric in
  supplied information \emph{by construction}, which is the point of the
  measurement and does not go away. The budget objection, which did, was closed
  by tripling both caps at $\nseq=2048$: the ratio moves $-0.16 \pm 0.18$ \\
Against a weight-shared (not merely unstructured) baseline the margin is
$2.3$--$6.1\times$ & settled &
  \texttt{exp\_31\_structured\_baseline.py}, sixteen seeds, all-site scored,
  provenance-clean; the weight-shared window head beat two architectures able
  to propagate further across the chain in the screening that selected it, so
  the confirmed baseline is the simplest one tested, not the strongest
  conceivable one \\
Local score estimation is near-optimal away from mid $t$ & settled &
  the windowed estimator is the exact conditional expectation under strictly
  stationary initialisation; the initial-law mismatch was measured at a median
  ratio of $1.0038$ and does not account for the effect. One exception: uniform
  innovations at $\corr=0.5$, $t=0.05$ \\
Higher-order structure is more fragile to noising & conditional &
  \emph{qualitatively} yes, on a family where $\beta$ moves the fourth moment at
  exactly fixed variance and after projecting out $(\corr,\ivar)$. The
  \emph{ratios} are not reportable: they run to $10^4$ and depend on the
  parametrisation, the nuisance projection and the endpoints. Lead with the
  analytical statement, not the table \\
\emph{Single-frame} marginals are blind to the ring's rotation & settled &
  theorem. Only one-frame marginals are blind; two-frame and larger joints are
  exactly where $\psi$ is identifiable, and the claim was previously stated
  over ``marginals'' without that qualifier. The numerical control is a
  machine-precision zero against a density function that takes no $\psi$
  argument, so it checks consistency, not the theorem \\
Non-Markov violations are not interchangeable & conditional &
  two violations probed, not a general theory \\
The \emph{shape-based} capacity effect is a convergence artefact & withdrawn &
  \S\ref{sec:corr-capacity}; most shape coordinates had not settled at the
  budget used, and ${\sim}96\%$ of the apparent effect is rate. What is
  withdrawn is the capacity \emph{attribution} for anything resting on fitted
  or generated shape \\
Held-out evidence saturates near eight components & withdrawn &
  \emph{reversed} by \texttt{exp\_32} (sixteen paired seeds, two sizes, fits run
  to plateau, predeclared equivalence region). No capacity improves on $C=1$ at
  either size, and the $C{=}8$ vs $C{=}16$ step resolves \emph{against} the
  region at $\nseq=128$ with $C=16$ worse. The six-seed, forty-iteration design
  that reported the opposite sign does not reproduce \\
...
EM stopping & relative $10^{-9}$ on the log-evidence; \textbf{never} on $\corr$. The cap is
whatever the caller passes --- \codefile{exp\_07} a literal $120$,
\codefile{exp\_32} $1200$ --- \emph{not} the \texttt{em\_max\_iters}
declared here, which nothing reads (see the box below) \\
ring & $T = 12$, so $\nsites = 24$ \\
\bottomrule
\end{tabular}
\end{center}
...
\begin{pitfall}
\textbf{``Both arms select their cap'' was read as ``the ratio is bounded by the cap''. It is
not, and the difference is measurable.} This box used to say that the largest ratio was the least
trustworthy number in the table, on the grounds that at $\nseq=2048$ the network selects the
$20{,}000$-step cap in $33\%$ of cells and EM--BP the $400$-iteration cap in $61\%$, and to advise
quoting $7.3$--$12.6$ rather than the endpoint.

That inference does not follow. An arm selects its last checkpoint whenever validation error is
still falling, at whatever rate; cap-selection therefore reports the \emph{sign} of the remaining
gain and says nothing about its \emph{size}. The size was measured directly: array
\texttt{631467} reruns $\nseq=2048$ with the EM cap tripled ($400 \to 1200$) and the network's
tripled ($20{,}000 \to 60{,}000$), on the same sixteen seeds and the same validation and test
bundles, so the comparison pairs seed by seed --- necessary here, because the seed-to-seed spread
in the ratio ($8.97$--$36.33$) is an order of magnitude larger than the effect sought.

The paired difference is $-0.16 \pm 0.18$ ($t=-0.87$ on $15$ df) and the network's error moves by
$-0.3\%$. Under the raised caps the network's cap-selection falls from $33\%$ to $6.8\%$ --- it
genuinely stops being budget-constrained --- while EM--BP still selects its cap in $48\%$ of cells,
so the caps are still \emph{selected}; they simply do not \emph{bind}. The endpoint is quotable and
the advice above is withdrawn.

The generator enforces this rather than trusting the prose: it recomputes the paired difference on
every run and refuses to emit the table if it exceeds $1.0$.
\end{pitfall}

\paragraph{$\nseq=4096$, and why it is in the table after being excluded.}
\texttt{frozen\_config} originally dropped $4096$ from \texttt{efficiency\_sizes} citing exactly
the reasoning the paragraph above retracts. With that reasoning gone, the size was run: arrays
\texttt{631496}/\texttt{631497}, sixteen seeds on H200, at a budget raised to match ($2400$ EM
iterations, $60{,}000$ steps). All $192$ cells are resolution-certified (minimum $s_{\min}/h =
5.07$), and EM--BP is more accurate in $192$ of $192$ --- the ratio is $17.60 \pm 1.21$, continuing
the rise with $\nseq$, at a mean transition Hellinger of $0.0457$.

Because that row runs at a different budget from the rest of the table, the $\nseq=2048$ rerun
above does double duty: it is the calibration that makes the two protocols comparable, and it is
disclosed as such in the caption.

\begin{pitfall}
\textbf{A cell-level split that was really a seed-level one.} At $\nseq=4096$ the cells where the
EM stopping rule fires look worse than the censored ones ($0.0110$ against $0.0089$ in score
error), which reads as the stopping rule cutting fits short. It is not: \texttt{stop\_reason} is
constant within a seed --- part 5 fits EM once per seed and evaluates it at all twelve noise
levels --- so the split is four seeds against twelve, not $48$ cells against $144$. At the seed
level the gap is not significant (Welch $t = +0.95$ on score error, $+1.49$ on Hellinger,
$-0.64$ on the ratio). This is the same error as the replicate-count episode in
\S\ref{sec:corr-replicates}, in a new costume.
\end{pitfall}
...
Two contamination mechanisms, both with an exact reference. \texttt{ratio\_to\_em} is the
baseline's relative score error over EM--BP's, so $>1$ means the estimator wins.

\input{../shared/sections/tab-nonmarkov}

\begin{pitfall}
The table above replaces a hand-typed one, sourced from run \texttt{627165}, whose numbers could
not be reproduced from any committed output under any aggregation --- and whose baseline had
trained for $8{,}000$ steps against the frozen protocol's $20{,}000$, so every ratio in it was
inflated by an undertrained comparator. It is now generated by
\codefile{tools/make\_tab\_nonmarkov.py} from the clean-deployment rerun described below
(arrays \texttt{645687} and \texttt{645903}), which reproduces the frozen-budget rerun
(array \texttt{633406}, \texttt{em\_iters=400}, \texttt{net\_steps=20000}) exactly on the
Gaussian arm and adds the Laplace one.

The correction moves the numbers in the direction the defect predicts: at $\beta=0$ the Gaussian
ratio against the CNN falls from $18.3$ to $16.41$, and against the MLP from $37.0$ to $27.84$.
A comparison against an undertrained baseline is not a conservative error, and the earlier
version of this section stated a scope conclusion on top of one.
\end{pitfall}
...
$\gamma\approx0.10$} under long-range coupling, where it now loses to both baselines rather than
to the CNN alone. The mechanism is exactly what the structure predicts: a global latent makes the
score residual rank one, which a chain absorbs; long-range coupling is not low-rank and cannot be
represented. This is the honest scope statement, and it is backed by a frozen-budget run.

\begin{pitfall}
The specific ratios here have their own noise schedule and training size and are not to be
pooled with \S\ref{sec:rec-efficiency}; only the direction and the crossing point transfer.
\end{pitfall}

\paragraph{Landed: the clean-deployment rerun} (\texttt{645687}, Laplace $\times5$;
\texttt{645903}, Gaussian $\times10$). The frozen-budget run above was correct in protocol and
uncertifiable in provenance: \codefile{src/em.py} was uncommitted in all ten cells, on the
executed code path, so \texttt{require\_clean} refused it even with \texttt{allow\_legacy}.
Both arms have now been rerun from git-archive deployments with empty dirty lists.

Two things came out of it, and the second was not the expected one.

The Gaussian numbers are unchanged --- every value the thesis quotes reproduces to the printed
precision. So the uncommitted file was, in fact, irrelevant to this experiment. That was always
the likely answer; the point is that it could not be \emph{asserted} before and can be now, and
the cost of finding out was two array jobs.

The Laplace arm is new: the frozen tree never contained one, so \S\ref{sec:rec-nonmarkov} had
been reporting a Gaussian-only result under a general heading. It mostly agrees --- same
ordering, same smooth decay --- but not entirely. At $\beta=1$ the Gaussian arm stays ahead at
every noise level, worst ratio $1.06$; the Laplace arm stays ahead of the CNN at worst $1.32$
and lands on the break-even line against the MLP at the largest $t$, at $0.998$. The claim that
survives is about the mechanism, not about the size of the margin, and the thesis now says so
in those terms.

\begin{pitfall}
The two arms were deployed separately and carry different commits, which
\codefile{tools/provenance\_gate.py} treats as a refusal condition --- pooling two programs into
one cell. It is not one here, and the generator proves rather than assumes it: no cell mixes
commits, and all sixteen files of \codefile{experiments/exp\_21\_nonmarkov.py}'s import closure
are byte-identical between the two. Both checks run on every regeneration. A mixed-commit table
that passes them is exactly as trustworthy as a single-commit one; one that does not is refused.
\end{pitfall}
...
\paragraph{Landed: the structure-aware baseline}
(\codefile{experiments/exp\_31\_structured\_baseline.py}, jobs
\texttt{641509}--\texttt{641514}, sixteen seeds, all three lanes complete,
provenance-clean at commit \texttt{569a67c}). The measurement an outside review
identified as the outstanding one: what survives of the headline ratio against
a baseline that can represent chain inference, rather than the fixed window
that closes most of it. Screened three architectures on development seeds ---
the weight-shared window head, a dilated convolutional stack, and a
bidirectional message-passing network, the last two able in principle to
propagate across the whole chain --- and \textbf{the window head won}, at
radius $4$, width $64$. Confirmed at the headline protocol on the full sixteen
frozen seeds, all-site scored: EM--BP is more accurate than the window head in
every one of $192$ scored cells --- $16$ seeds $\times$ $4$ sizes $\times$ $3$
regions, not the $128$ a two-column table suggests --- by a factor rising from
$2.34\times$ at $\nseq=32$ to $6.14\times$ at $\nseq=2048$. \emph{The radius was
wrong in three documents until 1 September.} It was typed as $2$, read off the
all-site aggregation rather than the bulk one the selection rule actually uses,
and stood in the thesis, the paper's appendix and this chapter until
\texttt{tools/make\_tab\_screening.py} recomputed it from
\texttt{screening.csv}. It is now a macro. The increase with $\nseq$ is
recorded as a description of four measurements and no longer offered as evidence
that the residual is non-architectural: an approximation floor produces the same
signature, and the raw risks --- EM--BP falling $4.5\times$ across the range
against the window head's $1.6\times$ --- are consistent with one. This
replaces the withdrawn \texttt{exp\_12\_scaled} comparison in every document
and closes the review's largest single open item. The table generator is
\texttt{tools/make\_tab\_structured.py}, rewritten for this run's schema and
refusing to run if the screened architecture ever changes or provenance ever
fails, rather than silently reporting a stale assumption.
...
\paragraph{Landed: capacity equivalence, and it reverses the claim it was built
to test.}
      (\codefile{experiments/exp\_32\_capacity\_equivalence.py}.) Predeclared equivalence region,
\texttt{em\_cap} raised from $400$ to $1200$. The raise is itself a measurement rather than
a convention: at the old cap every one of the $320$ fits was still censored, and a
representative $\ncomp=16$ fit was moving thirty times faster than $\ncomp=8$ over the last
hundred iterations --- the same capacity-versus-convergence-rate confound, one level up. Run
to $2000$ iterations, the per-edge gain falls to ${\sim}6\times10^{-9}$ by $800$ and then
plateaus, so $1200$ captures essentially all the real movement without pretending the strict
$10^{-9}$ tolerance is reachable. Jobs \texttt{641515}--\texttt{641520} plus the resume shard
\texttt{643778}; lane~b had reached $39/50$ when two consecutive six-hour shards ran out of
wall time, and \texttt{part\_sweep}'s per-cell flush and on-disk resume check made a third
shard safe against the same output directory rather than destructive to the first
thirty-nine cells. Complete at $160/160$: sixteen seeds, five capacities, two sizes.

The result is not the one the design was hoping to be able to state. Paired against
$\ncomp=1$ on identical data, \emph{no} capacity improves held-out evidence or
schedule-level score risk at either size --- $\ncomp=1$ wins on $13/16$ seeds at
$\nseq=128$ and $14/16$ at $\nseq=512$ --- and the predeclared region refuses the
$\ncomp{=}8$ versus $\ncomp{=}16$ comparison as well, resolving against it at $\nseq=128$
with $\ncomp=16$ measurably worse. The six-seed, forty-iteration table it supersedes
reported the opposite sign and was read as capacity beginning to pay at $\ncomp=4$; that
does not reproduce. The likely mechanism, offered but not tested, is that the short budget
was regularising the larger mixtures. Two confounds remain and both disfavour high capacity:
every cell at $\ncomp\geq4$ stopped at the cap rather than at tolerance, and the share of
fits whose narrowest component falls under the grid's resolution floor climbs from $0/32$ at
$\ncomp=1$ to $11/32$ at $\ncomp=16$. Separating them needs a finer grid at fixed capacity
and a budget ladder at fixed capacity, neither run.

One artefact from this rerun must not be used. A contrast job (\texttt{639099}) fired early
because its \texttt{afterany} dependency was satisfied by the \emph{cancellation} of a
restructured upstream shard --- \texttt{afterany} fires on any terminal state and cannot
distinguish ``the work finished'' from ``something happened to that job id''. It computed
the contrast on seeds $0$--$6$ alone, roughly $40\%$ of the design. The file is renamed
\texttt{DO\_NOT\_CITE\_premature\_contrasts\_job639099.csv} on the cluster so that a
merge script globbing the ordinary name finds nothing; it is kept rather than deleted
because its visibly over-wide $\nseq=512$ interval is the demonstration of why the full seed
count matters.
...
\citep{mei2024unets,garnierbrun2024transformers}. Which architectures exploit Markovianity, and at
...
\subsection{Pointwise accuracy against generative fidelity}
\label{sec:corr-dissociation}

\paragraph{The claim.} That pointwise denoising accuracy and generative fidelity
\emph{dissociate}: that an estimator can be an order of magnitude better at predicting
$\E[a \mid x]$ and simultaneously worse at generating samples with the right innovation
shape. This was given as the central finding of the results chapter in circulated drafts of
both the thesis and the note.

\paragraph{What was wrong.} The same $40$-iteration budget as \S\ref{sec:corr-capacity},
applied to the one statistic that needed more. The fitted innovation \emph{shape} settles at a
median of $229$ updates and as late as $638$, while $\corr$ settles at a median of $80$
(the settling table above), and the generative metric is
a shape statistic. So the comparison scored converged estimators
against unconverged ones on precisely the coordinate that had not converged, and the ranking
it produced is not attributable to the estimators.

This is a confound rather than a refutation --- the dissociation may well be real --- but the
experiment as run cannot distinguish it from the iteration budget, and that is enough to
withdraw the claim. It is withdrawn and \textbf{unreplaced}: no corrected version has been
run at a budget that would settle the question.

\paragraph{The numbers, for the record.} Generated-sample residual excess kurtosis against
the closed-form target of $1.9098$ derived in \S\ref{sec:corr-generation}, mean over four
seeds per cell, under the corrected forward-noising protocol (the two network-free arms are
bit-identical to the earlier protocol, and both trained networks improve under it).
...
\noindent The within-arm version was the sharpest form of the claim, and is worth recording
because it needs no comparison between estimators: at $\nseq=2048$ the global MLP's four
per-seed generated-kurtosis values were $0.097$, $0.069$, $0.019$ and $-0.002$ --- all
indistinguishable from a Gaussian generated law --- while its own pointwise error improved
monotonically over the same range. One estimator disagreeing with itself across two notions
of correctness as its training data grows. The $40$-iteration confound applies to the arms it
is compared against, and the proposed mechanism (a better $L_2$ fit to the posterior mean is
smoother, and a smoother denoiser drives the reverse SDE toward a more Gaussian generated
law) was never measured directly.

\paragraph{The check that would have caught it.} Any convergence diagnostic on the reported
statistic itself. Monitoring $\corr$ --- the natural thing to plot, and what was plotted ---
certifies a run whose shape is still moving, because the two coordinates converge at rates
differing by a factor of eighty. The lesson generalises past this claim: a convergence check
must be run on the quantity being reported, not on the parameter that is easiest to watch.
...
\paragraph{What was wrong.} In \codepath{src/kernels.py}, the method
\texttt{\small MixtureInnovationKernel.\allowbreak
grad\_\allowbreak log\_\allowbreak transition\_\allowbreak matrix} carried a
sign error in the $\corr$ derivative.
...
\section{Provenance history displaced from the reproducibility appendix}
\label{sec:provenance-history}

The thesis's Appendix~C states the three certification routes and the current
status of each experiment. What it no longer carries, because it is development
record rather than protocol, is why each route exists. That is here.

\paragraph{Why \texttt{exp\_21} needed a rerun.} The tree it originally ran
from, \texttt{outputs/exp\_21\_frozen}, had \texttt{src/em.py} uncommitted in
all ten cells. The dirt was on the executed code path, so the recorded commit
did not reconstruct the program that produced the numbers --- the same defect as
\texttt{exp\_12\_scaled} and the withdrawn $\nseq = 8192$ efficiency row.
\texttt{make\_tab\_nonmarkov.py} declared it in the table's own caption rather
than emitting something a reader could not distinguish from the certified
tables, and the section was to be read against a rerun until that rerun landed.

What the rerun then settled is worth recording, because it was not knowable
beforehand: the Gaussian arm reproduced the run it replaced on every quoted
number to the precision printed. The uncommitted file had never been on this
experiment's path in any way that mattered. The gate refused a run that was, in
the event, correct --- which is the behaviour a provenance gate should have,
since the alternative is a gate that passes whenever the dirt happens to be
harmless and therefore certifies nothing.

\paragraph{Why the mixed-commit check exists.} The rerun was deployed in two
git-archive batches, Laplace and Gaussian, so it spans two commits. A cell
pooling two commits would be uninterpretable, and two commits differing anywhere
on the import closure would make the arms incomparable. The generator therefore
refuses unless no single cell pools commits and all sixteen closure files are
byte-identical across them. Neither condition was checked by hand; both run on
every regeneration.

\paragraph{Why the last hand-typed number was removed.} The screening winner's
radius was the final quantity in the architecture section still typed rather
than generated, and it was wrong in three documents at once --- thesis, paper
and this compendium all said radius~2, while the selection rule the code
implements picks radius~4. \texttt{make\_tab\_screening} now recomputes it from
the artifact. A number that appears in three places and is maintained in none of
them is the failure mode the generated-macro rule exists to prevent.

\paragraph{Why the $\nseq = 8192$ exclusion is enforced in code.} The thesis's
Appendix~C states which row is withdrawn and why. What belongs here is that the
check is not a one-off: \texttt{make\_tab\_efficiency.py} recomputes the
intersection of each run's dirty list with its own import closure on every run
and refuses if it is ever non-empty. The figure beside the table reads the same
size grid from the same source, so the two cannot come to disagree about which
sizes are reportable --- which they once did, the table stopping at $4096$ while
the curve beside it ran to $8192$.
