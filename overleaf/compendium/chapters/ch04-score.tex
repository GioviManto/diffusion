...
Verified in \codepath{tests/test\_sample\_metrics.py}, by
\texttt{test\_score\_reweighting\_matches\_the\_tweedie\_factor} and
\texttt{test\_reweighting\_spans\_the\_claimed\_range\_across\_the\_schedule}.
\end{codebox}
\end{pitfall}
...
The identity between the score of a Gaussian-convolved density and the posterior mean is
classical in empirical Bayes, due to \citet{robbins1956empirical} and \citet{miyasawa1961empirical}, and is
usually called Tweedie's formula after the exposition in \citet{efron2011tweedie}. Its use as a
training objective for generative models is \citet{vincent2011connection}, building on the score-matching
estimator of \citet{hyvarinen2005estimation}; the generative-modelling application is \citet{song2019generative}.
