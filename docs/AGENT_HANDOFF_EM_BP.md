...
../../.venv/bin/python -m pytest tests/ -q          # expect 35 passed, ~2.5 min
```
...
6. **The Laplace kernel has two discretization artifacts** (compendium §5.1, §5.2): its exact M-step is quantized onto the grid's ratio lattice, and its ρ-gradient loses accuracy to a `sign` discontinuity. **Never quote a Laplace-kernel ρ recovery as evidence of accuracy.** Measured (compendium §4.10): at M=201 three distinct true values — 0.770, 0.800, 0.813 — *all* return exactly 0.750, and refining the grid plateaus the error near 0.008 instead of driving it to zero. The estimator is confined to a lattice of small-denominator rationals; which point it lands on depends on the data, so the 1e-16 "recovery" in Part 1 (N=1024) and the 0.022 error in Part 5 (N=256) are the same phenomenon at two sample sizes, not a good result and a bad one. Use the smooth kernels (Gaussian, mixture) for any rate or accuracy claim.
...
In rough priority order. Items 1–2 are repairing results that did not come out
clean; 3–4 strengthen the headline; the rest are new work.
...
2. **Fix the exp_06 Part 5 seeding confound.** `rng_for("exp06-p5", rho_true, m_size)` makes each grid size draw different data, so the across-`M` trend and the entire `b`-error column are uninterpretable. Drop `m_size` from the seed key so the grid is the only thing varying, then re-run. The within-`M` result (three distinct ρ\* all collapsing to 0.750 at M=201) stands regardless.
3. **Error bars on the headline curve.** `hpc/slurm_replicates.sbatch` + `tools/merge_replicates.py`. The current curve has one replicate per `N`, which is why the EM line wobbles. This is the single highest-value cluster run.
4. **Extend the sample-efficiency sweep upward.** The network was still improving at `N = 2048`, so "EM-BP needs ≥64× less data" is a lower bound read off a truncated curve. Follow it until the network catches up or visibly plateaus — either outcome is publishable, and the honest version needs it.
5. **A structured-architecture baseline** (temporal CNN / small U-Net). This is the most important open question for the paper's credibility: the current baseline is a vanilla MLP, as the email specified, and a reviewer will ask. Expect it to close part of the gap; it will not acquire the exactness or the uniformity in `t`.
6. **Inference-time cost.** BP is 211×–320× slower per evaluation than a forward pass, and reverse diffusion calls the denoiser at every step. Either distil the fitted BP denoiser into a network, or use Layer-2 Gaussian-projected BP at inference and measure what the closure costs. This is the weakest point of the whole story.
7. **Hybrid non-Markov correction.** Layer 4 showed the score correction is exactly rank one for AR(1)+global-latent priors. Combining that with a *learned* chain kernel is the natural Layer 4 × Layer 5 product and is untouched.
8. **The write-up (advice 2 in the original email).** `report/em_bp_learning.tex` is deliberately paper-shaped — context, propositions, detail in remarks — and is the natural seed for the shared Overleaf document. It has no results section yet; §4 of the compendium is the raw material.
