...
def per_seed_ratio(region, n):
    ...
    for s in seeds:
        ...
        wn = [F(r, "risk") for r in rows if r["seed"] == s
              ...
              and r["region"] == region]
        assert len(em) == 1 and len(wn) == 1, (s, n, region, em, wn)
        ratios.append(wn[0] / em[0])
    v = np.array(ratios)
...
for n in sizes:
    ...
    lines.append(f"{n:<4} & ${ratio:.2f} \\pm {se:.2f}$ & "
...
Ratio is risk(window)/risk(EM--BP), paired per seed, {len(seeds)} seeds
