...
\end{equation}
Experiment~08 measures them side by side, and the answer is not one-sided. On
the smooth Gaussian kernel a well-chosen rate is fully competitive (it reached a
marginally \emph{higher} likelihood than EM in the same number of iterations,
both converged), while a rate four times larger diverges and violates
monotonicity by tens of nats. On the Laplace kernel the gradient route actually
recovers $\rho$ \emph{better} than the exact M-step --- because the exact
M-step is the quantized one (Remark~\ref{rem:quantization}), and gradient steps
move continuously --- while doing worse on the scale $b$ and giving up exact
monotonicity.

So the two are complementary rather than ranked. The exact M-step is
step-size-free and exactly monotone, and should be preferred wherever it exists
and the kernel is smooth; the gradient route earns its place where the M-step
has no closed form (Rung 4) or where its exact solution is a discretization
artifact (Rung 2). What is common to both, and is the point of
Proposition~\ref{prop:fisher}, is that neither ever differentiates through the
...
On the Laplace chain with $400$ observed noisy chains, the neural kernel
($873$ parameters) reaches a \emph{higher} training log-likelihood than the
mixture kernel of Rung 3 ($13$ parameters) --- $-16652$ against $-16656$ --- and
a \emph{worse} held-out denoiser: relative score error $0.052$ against $0.026$.
That is ordinary overfitting, and it is worth stating plainly because the
temptation this note is arguing against --- reach for the network --- reappears
one level down. Rung 4 is the right tool when the transition genuinely has
state-dependent shape that Rung 3 cannot express; on data where a linear
autoregression with a free innovation law is correct, it buys nothing and costs
two orders of magnitude in parameters. The efficiency argument of
Section~\ref{sec:efficiency} is an argument for matching the hypothesis class to
the structure, not an argument that neural components are inadmissible.
\end{remark}
