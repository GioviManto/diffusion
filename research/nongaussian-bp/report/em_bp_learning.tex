...
that pool weight onto it. Experiment~06 Part~5 measures the consequence with
the data held fixed so that the grid is the only thing varying, and it is worse
than a resolution limit: $4/5$ acts as an \emph{attractor}. For
$\rho^\star = 0.7913$, chosen deliberately off the simple lattice, $\hat\rho$ is
pinned at exactly $0.8000$ across an eightfold grid refinement
($M = 201 \to 1601$), holding a constant bias of $0.0087$; at $M = 201$ three
distinct true values collapse onto $0.8000$. Refining the grid does not help,
because this is a bias and not a discretization error that vanishes with $dx$.
It also contaminates the scale: where $\rho$ is snapped away from the truth the
error in $b$ is three to four times larger. The reported rates accordingly use
the smooth kernels, whose M-steps are ratios of smooth moments and vary
continuously.
\end{remark}
...
denoiser at \emph{every} noise level simultaneously. Its antecedent is
measured rather than assumed: experiment~06 Part~4, at twelve replicates,
gives log--log slopes whose inverse-variance combination is
$-0.500 \pm 0.048$, excluding $-0.25$ and $-0.75$ at more than three standard
errors even after inflating the error to account for the estimates sharing
fits. At four replicates the same experiment could not resolve the slope to
better than roughly $\pm 0.2$ and did not support the claim --- a reminder that
a rate needs replicates in the tens. The network gets no such
statement; its error must be paid for separately in every region of $(x,t)$ it
is asked about.

It is worth recording that the obvious corollary of this --- that the network
should visibly break at noise levels outside its training schedule --- is
\emph{false}, and experiment~07 Part~3 was designed expecting it. Across
thirteen levels from $t = 0.02$ to $t = 3.2$ the network shows no cliff at the
schedule boundary, and the EM-BP advantage (a factor of $4.5$ to $19$) does not
separate in-schedule from out-of-schedule levels at all. Diffusion time enters
through smooth features and the target moves smoothly with $t$, so a network
handles that one direction adequately: the estimation difficulty lives in
$x$-space, not in $t$-space. What survives is the weaker but real statement
that EM-BP has no schedule to leave --- its error varies smoothly across the
whole range because its parameters are properties of the clean chain and BP
supplies each $t$ exactly.
...
\item \textbf{Inference cost is the honest loss.} Grid BP costs $O(nM^2)$ per
  ...
  detail. The measured slowdown is $211\times$ to $320\times$ at $M = 401$
  (experiment~07 Part~4, $1.2$--$2.0$\,ms per chain against $4$--$10\,\mu$s for a
  forward pass) and is reported as prominently as the wins. The natural
